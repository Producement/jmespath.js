    ...
    it('should accept custom functions', function () {
        assert.deepEqual(
            jmespath.search({ x: [5, 6, 7] }, "example(x[1])", {
                resolveUnknownFunction: function (name) { return { _func: function (args) { return args[0] * 2 }, _signature: [{ types: [0] }] }; }
            }),
            12);
    });
    it('should validate custom function signature', function () {
        assert.throws(
            () => {
                jmespath.search({ x: [5, 6, 7] }, "example(x[1])", {
                    resolveUnknownFunction: function (name) { return { _func: function (args) { return args[0] * 2 }, _signature: [{ types: [2] }] }; }
                })
            }, { name: 'Error', message: 'TypeError: example() expected argument 1 to be type string but received type number instead.' });
    });
});
